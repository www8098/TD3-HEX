...
class TD3(object):
    def __init__(self, nb_states, nb_actions, args, noise):
        ...
        self.actor = Actor(self.nb_states, self.nb_actions, **net_cfg)                  # input state, hiden1, hiden2, output ActionNoise
        self.actor_target = copy.deepcopy(self.actor)
        self.actor_optim  = Adam(self.actor.parameters(), lr=args.rate, eps=1e-08, weight_decay=args.L2)

        self.critic1 = Critic(self.nb_states, self.nb_actions, **net_cfg)                # self.fc2 = nn.Linear(hidden1+nb_actions, hidden2)
        self.critic1_target = copy.deepcopy(self.critic1)
        self.critic1_optim  = Adam(self.critic1.parameters(), lr=args.rate, eps=1e-08, weight_decay=args.L2)

        self.critic2 = Critic(self.nb_states, self.nb_actions, **net_cfg)                # self.fc2 = nn.Linear(hidden1+nb_actions, hidden2)
    ...
    def update_policy(self, step: int):
        ...
        noise = torch.Tensor(self.nb_actions).data.normal_(0, self.policy_noise).cuda()
    ...
    def select_action(self, s_t, decay_epsilon=True):
        ...
        # ActionNoise = (ActionNoise + 0.3*np.random.original-TF3(0, self.expl_noise, size=self.nb_actions)).clip(-1, 1)
        if self.noise:
